...
export class UtilsString {
    ...
    public static readonly EMAIL_IS_VALID_PATTERN = "^(?=.+[@].+).*$";
    public static readonly WEB_ALIAS_PATTERN = '[^\\s]+';
    ...
    public static GetMoneyFormattedOrDefaultString(val: number,
                                                   defaultRes: string = ''): string {
        val = +val;
    ...
    public static GetTimePattern(): string {
        let p1 = "[012]{1}";
        let p2 = "(([01]{1}[0-9]{1})|([2]{1}[0-3]{1}))";
        let p3 = p2 + ":{1}";
        let p4 = p3 + "[0-5]{1}";
        let p5 = p4 + "[05]{1}";
    ...
    public static GetTypeFromString(string) {
        return typeof UtilsString.GetCorrectVariable(string);
    }

    public static GetCorrectVariable(string): any {
        string = string.trim();
        if (!string) return "";
        ...
        if (string === "true") return Boolean(true);
        if (string === "false") return Boolean(false);
